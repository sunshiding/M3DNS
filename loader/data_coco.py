...
def load_data(path,hp):
    ...
    permutation = np.load(path + 'permutation.npy')
    text = np.load(path+'coco_text.npy')
    files = pickle.load(open(path+'coco_imgs.pkl','rb'))
    
    def get_single_data(idx):
        target = label[idx,:]
        file = files[idx]
        file_path = path + 'train2014/'+file
